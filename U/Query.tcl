...
namespace eval ::Query {
	...
	if {[dict exists $r -Url query]} {
	    # parse the query part normally
	    Debug.query {parsing query part ([dict get $r -Url query])}
	    lassign [qparse [dict get $r -Url query] 0] query count
	} else {
	    set query {}
	    set count 0
	}
	...
	while {[set offset [string first "$lineDelim--$boundary" $query $offset]] >= 0} {
		...
		switch -- $te {
		    ...
		    base64 {
			set content [binary decode base64 $content]
		    }
		    7bit - 8bit - binary - "" {}
		    default {
...
if {[info exists argv0] && ([info script] eq $argv0)} {
	...
	set query [Query parse [list [-Url [list query $test]]]]
	puts stderr "'$test' -> ($query)"
	puts stderr "find error '[Query value $query error]'"
